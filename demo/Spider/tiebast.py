...
class TiebaSpider:
    ...
    def get_content_list(self,html_str): #提取数据
        ...
        for div in div_list:
            ...
            item["href"] = self.parse_url+div.xpath("./a/@href")[0] if len(div.xpath("./a/$href"))>0 else None
            item["img_list"] = self.get_img_list(item["href"],[])
            content_list.append(item)
        #提取下一页的url地址
        next_url = self.parse_url+html.xpath("//a[text()='下一页']/@href")[0] if len(html.xpath("//a[text()='下一页']/@href")) else None
    ...
    def get_img_list(self,detail_url,totol_img_list): #获取帖子中的所有图片
        ...
        if len(detail_url)>0:
            detail_next_url = self.parse_url+detail_next_url[0]
